 ...
 * If the database is unreachable, silently falls back to JSON mode.
 */
...
if (defined('USE_JSON') && USE_JSON) {
    ...
    try {
        ...
        // Database unavailable — fall back to JSON for tests / local dev
        $repo = new JsonCarRepository($dataDir);
    }
}
...
/* ========================================================================
   ...
   These functions preserve the old flat-file API while the project
   transitions to Controllers. They will be removed once all pages
   route through app/Controllers/.
   ======================================================================== */
