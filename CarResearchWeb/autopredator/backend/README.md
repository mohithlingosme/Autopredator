...
- AI feature flags: `AI_ENABLED`, `AI_CONTENT_ENABLED`, and `AI_SUPPORT_ENABLED` (all default to `false` unless set in env). Toggle them in `.env` to safely roll out AI features.
- Database constants live in `config.php`, but JSON is the active datastore.

## Database Setup
The application supports both JSON data sources (legacy) and MySQL database (`autopredator_cars`).

### Database Migration
1) Apply the hardening migration: `mysql -u root -p < db/migrations/2026-01-08_autopredator_cars_hardening.sql`
2) Run verification: `mysql -u root -p autopredator_cars < db/checks/verify_constraints.sql`
3) Seed data using idempotent scripts in `db/seeds/`
...
USE_JSON=false  # Set to false to use database
```

## Data Layout
- **Database Mode**: Uses `autopredator_cars` schema with brands, models, variants, images, features, and pricing.
- **JSON Fallback**: `data/new_carset.json` — primary dataset (make/model/segment/variants with pricing and fuel).
...
### Adding Data
1) **Database Mode**: Use seed scripts in `db/seeds/` with UPSERT patterns for idempotent imports.
2) **JSON Mode**: Drop new model entries into `data/new_carset.json` (or legacy `data/data.json`).
3) If you add a per-model detail sheet, place it under `data/` and register it in `data/details_index.json` with a slug matching `/^[a-z0-9]+(?:-[a-z0-9]+)*$/`.
4) Clear PHP opcache if enabled, then reload the page.
