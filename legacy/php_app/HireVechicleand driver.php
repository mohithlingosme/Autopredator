...
<body class="bg-gray-50 text-gray-800">
  ...
  <section class="max-w-7xl mx-auto py-10 px-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <?php
      // Fetch vehicles
      $vehicles_query = "SELECT id, name, location, price_per_day, image FROM vehicles LIMIT 2";
      ...
      if ($vehicles_result->num_rows > 0) {
          while ($vehicle = $vehicles_result->fetch_assoc()) {
              ...
              echo '<img src="' . htmlspecialchars($vehicle['image']) . '" alt="Vehicle" class="rounded h-40 w-full object-cover mb-4" />';
              echo '<h3 class="text-lg font-bold">' . htmlspecialchars($vehicle['name']) . '</h3>';
              echo '<p>Location: ' . htmlspecialchars($vehicle['location']) . '</p>';
              echo '<p>₹' . htmlspecialchars($vehicle['price_per_day']) . '/day</p>';
              echo '<button class="mt-4 bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Hire Now</button>';
              echo '</div>';
          }
      } else {
      ...
      // Fetch drivers
      $drivers_query = "SELECT id, name, experience, price_per_day, image FROM drivers LIMIT 1";
      $drivers_result = $conn->query($drivers_query);
      if ($drivers_result->num_rows > 0) {
          while ($driver = $drivers_result->fetch_assoc()) {
              echo '<div class="bg-white shadow p-4 rounded">';
              echo '<img src="' . htmlspecialchars($driver['image']) . '" alt="Driver" class="rounded h-40 w-full object-cover mb-4" />';
              echo '<h3 class="text-lg font-bold">' . htmlspecialchars($driver['name']) . ' (Heavy Vehicle Driver)</h3>';
              echo '<p>Experience: ' . htmlspecialchars($driver['experience']) . ' years</p>';
              echo '<p>₹' . htmlspecialchars($driver['price_per_day']) . '/day</p>';
              echo '<button class="mt-4 bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700">Hire Driver</button>';
              echo '</div>';
          }
      } else {
